...
describe('update', () => {
  it('should update a container config', async () => {
    db.query = jest.fn().mockResolvedValue({
      rows: [{ container_key: 'iuh23497iufg98q2irhas98df134', name: 'test', description: 'test', active: true, admin_only: false }]
    });

    const res = await repo.update('iuh23497iufg98q2irhas98df134', 'test');
    expect(res).toEqual({ container_key: 'iuh23497iufg98q2irhas98df134', name: 'test', description: 'test', active: true, admin_only: false });
  });
